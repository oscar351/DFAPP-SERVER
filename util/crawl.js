...
async function fetchAdventureData(advenName) {
    ...
    const results = await page.evaluate(() => {
        ...
        rows.forEach(row => {
            ...
            const set_point = row.querySelector('.seh_addinfo .saninf-setp')?.innerText.trim() || '';
            const switcing = row.querySelector('.seh_addinfo .saint-sumofswithing')?.innerText.trim() || '';
